...
monthly_sentiment<-df1b %>% group_by(Year_Month,Sentiment) %>% summarise(Count_Sentiment = n()) %>% ungroup() %>% group_by(Year_Month) %>% mutate (Month_Sum=sum(Count_Sentiment)) %>% ungroup() %>% mutate(Percent_in_Year = percent(Count_Sentiment/Month_Sum))
...
monthly_sentiment<-monthly_sentiment%>% mutate(Perc_Num=as.double( strsplit(Percent_in_Year,split = "%")))
...
### 6.3.3. Sentiment Bar Graph of All Tracks
 
Sentiment count of all unique songs.
...
### 6.3.4. Sentiment Gradient Chart
