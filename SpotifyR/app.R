...
ui <- fluidPage(
    ...
    sidebarPanel(
        ...
        checkboxGroupInput("country_id","Select Countries",choices = c("User",as.character(top_50_charts$country)), selected = c("Turkey","UnitedStates","Japan"))
    ),
...
server <- function(input, output) {
    ...
    output$radarPlot <- renderChartJSRadar({
        ...
        df_normalized <- cbind(df_combined[1], 
                               apply(df_combined[-1],2,
                                     function(x){(x-min(x)) / diff(range(x))}))
        
        #Radar Chart
        Features_df<- df_normalized[df_normalized$country %in% input$country_id,]
        #Normalizing Data
